module Twitterwatch
    module Modules
      module Statuses
        class Sentiment < Twitterwatch::Module
          self.meta = {
            ...
            :options     => {
              "THREADS" => {
                :value       => Twitterwatch::Concerns::Concurrency::DEFAULT_THREAD_POOL_SIZE,
            ...
            statuses.each do |status|
              threads.process do
                begin
                  ...
                  case sentiment
                  when :positive
                    info("Positive: ".bold.light_green + Twitterwatch::Util.excerpt(status.text, 80))
                  when :negative
                    info("Negative: ".bold.light_red + Twitterwatch::Util.excerpt(status.text, 80))
                  else
                    info(" Neutral: ".bold + Twitterwatch::Util.excerpt(status.text, 80))
                  end
