module Twitterwatch
    module Modules
      module Users
        class KloutInfluence < Twitterwatch::Module
          self.meta = {
  ...
  Klout ID from the #{'users/klout_id'.bold} module. It also requires that Twitterwatch has
  ...
  INFO
          ...
          def run
            if !klout_client
              error("Twitterwatch has not been configured with any Klout API keys")
              return false
            end
                  ...
                  if influence = klout_client.get_influence(user.klout_id)
                    influence[:influencers].each do |screen_name|
                      db_influencer = influencer_mutex.synchronize do
                        Twitterwatch::Models::Influencer.find_or_create(:workspace_id => current_workspace.id, :screen_name => screen_name)
                      end
                    ...
                    influence[:influencees].each do |screen_name|
                      db_influencee = influencee_mutex.synchronize do
                        Twitterwatch::Models::Influencee.find_or_create(:workspace_id => current_workspace.id, :screen_name => screen_name)
                      end
