module Twitterwatch
    module Commands
      class Workspace < Twitterwatch::Command
        self.meta = {
  ...
  There will always be a default workspace with the name #{Twitterwatch::Models::Workspace::DEFAULT_WORKSPACE_NAME.bold} which might be enough
  if you plan to use Twitterwatch for a small group of Twitter users.
  ...
  USAGE
        ...
        def list_workspaces
          longest_workspace_name = Twitterwatch::Models::Workspace.all.map(&:name).max_by(&:length)
          info("Available workspaces:\n")
          Twitterwatch::Models::Workspace.order(:name).each do |workspace|
        ...
        def select_workspace(name = nil)
          ...
          if workspace = Twitterwatch::Models::Workspace.first(:name => name)
        ...
        def create_workspace
          ...
          if Twitterwatch::Models::Workspace.first(:name => name)
            error("There is already a workspace with that name")
            return false
          end
  
          workspace = Twitterwatch::Models::Workspace.create(
            :name        => name,
        ...
        def rename_workspace
          ...
          if old_name == Twitterwatch::Models::Workspace::DEFAULT_WORKSPACE_NAME
            error("Default workspace cannot be renamed")
            return false
          end
  
          if !old_workspace = Twitterwatch::Models::Workspace.first(:name => old_name)
            error("There is no workspace named #{old_name.bold}")
            return false
          end
  
          if Twitterwatch::Models::Workspace.first(:name => new_name)
        ...
        def delete_workspace
          ...
          if workspace = Twitterwatch::Models::Workspace.first(:name => name)
            ...
            if workspace.default_workspace?
              self.current_workspace = Twitterwatch::Models::Workspace.create_default_workspace!
              return
            end
            if current_workspace.id == workspace.id
              self.current_workspace = Twitterwatch::Models::Workspace.first(
